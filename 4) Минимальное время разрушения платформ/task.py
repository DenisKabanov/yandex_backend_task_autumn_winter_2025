...
def main():
    with open("input.txt", 'r') as input_file, open("output.txt", 'w') as output_file: # открываем файлы для чтения и записи
        ...
        # если не пуста, то пока пока в ней есть элементы и самый первый (левый) элемент имеет значение меньше текущего рассматриваемого — удаляем левый элемент из очереди (с обновлением соответствующих данных в time как j-i на позиции i) и повторяем этот шаг, пока не нарушим условие (очередь опустеет, либо встретится элемент со значением больше рассматриваемого)
        ...
        N = int(lines[0])
        time = [-1] * N # список под время разрушения платформ
